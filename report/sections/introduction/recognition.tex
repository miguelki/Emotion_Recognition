...
\vspace{\baselineskip}
\noindent The Cohn-Kanade AU-Coded Facial Expression Database was created for research purpose in the area of automatic facial image analysis. Two versions of this database are at the moment published \cite{CK}.
\newline

\noindent The first version is the original release of the database. It is composed of 486 image sequences of 97 subjects. The database is FACS coded. For each sequence, the initial image is the neutral expression and the final image is a peak expression. For each emotion, a label is given. This label refers to the facial expression that is supposed to be expressed but not the one that is really expressed. This is the difference between the first and second version. The second version uses validated emotion labels \cite{CK}.
\newline

\noindent The second version called \textit{CK+}, is composed of both posed and spontaneous expressions. Concerning the posed expressions, the $22\%$ of the total number of image sequences is added and $27\%$ of the total number of subjects is added. An example of images contained in the database is given in Figure~\ref{ck_facs_coded}, where a female subject expresses the \textit{disgusted} emotion. The Figure~\ref{ck_facs_coded} shows all the step of the image sequence for the \textit{disgusted} emotion \cite{CK}.
\newline
