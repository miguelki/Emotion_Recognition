...
=======
\noindent The regions obtained when the sample image is partitioned do not have the same contribution to the expressed emotion. For each region, a weight is then assigned as in Figure~\ref{lbp_region_weight}. Thus, the histogram of each region is multiplied with a corresponding weight. 
\newline

\noindent For example, eyes bring more information about an emotion than cheeks. The face image contains also some background around the face and the regions that are at the border of the image and that represent the background have less weight than the one containing the face. As it is shown in the Figure~\ref{lbp_region_weight}, the borderline weights are all of 1. It is because these regions are the background and do not have importance into the facial expression recognition system. All the weights of the regions characterizing the face are superior or equal to 2. The regions that have the highest weights are the region of the eyes and of the mouth. 
>>>>>>> f3a1177041b834dbad80cc320be587f28fef16e6
\newline
